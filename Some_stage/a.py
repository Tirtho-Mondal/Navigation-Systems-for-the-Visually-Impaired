...
def make_patch_visual(patch01, path_cells, cell_px=28):
    """Visualize the 15x15 patch with yellow points instead of red lines."""
    R, C = patch01.shape
    ...
    # Draw yellow points for each cell in the path
    for rc in path_cells:
        center = cell_center(rc)
        cv2.circle(vis_rgb, center, radius=5, color=(0, 255, 255), thickness=-1)
...
def process_image_array(img_rgb, depth_net, clahe_clip, clahe_tiles):
    ...
    with measure(timings, "patchify"):
        ...
        # Plot patch image with axes
        plt.figure(figsize=(6, 6))
        plt.imshow(patch01, cmap="gray", origin="upper")
        plt.title("15x15 Patch Image")
        plt.xlabel("Column Index")
        plt.ylabel("Row Index")
        plt.colorbar(label="Normalized Intensity")
        plt.grid(True, linestyle="--", linewidth=0.5, alpha=0.7)
        plt.savefig("patch_image_with_axis.png", bbox_inches="tight")
        plt.close()
