...
def process_image_array(img_rgb, depth_net, clahe_clip, clahe_tiles):
    ...
    return {
        ...
        "total_s": total,
        "images": images,
        "path_cells": path,   # <-- include full path
        "start": start,       # optional: expose start
        "goal": goal,         # optional: expose goal
    }
...
def print_pipeline_report(image_label, result):
    ...
    # NEW: path points
    path = result.get("path_cells", [])
    print(f"  Path ({len(path)} points): {path}")
    if result.get("start") is not None and result.get("goal") is not None:
...
def process_api():
    ...
    # Ensure tuples are JSON-friendly (lists)
    path_list = [list(p) for p in result.get("path_cells", [])]

    return jsonify({
        "direction": str(result["direction"]),
        "slope_m_img": result["slope_m_img"],
        "path": path_list,
        "start": list(result.get("start")) if result.get("start") is not None else None,
        "goal": list(result.get("goal")) if result.get("goal") is not None else None
    })
